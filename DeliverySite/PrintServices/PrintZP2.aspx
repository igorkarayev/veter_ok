<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PrintZP2.aspx.cs" Inherits="Delivery.PrintServices.PrintZP2 "%>
					...
					<ItemTemplate>
						<div id="Tr2" runat="server" style="width:20.7cm; height: 14.85cm !important; overflow: hidden !important; font-size: 8px !important; font-family: sans-serif">
							...
							<div style=" margin-top: -5px; width: 20.5cm; display: inline-block; vertical-align: top">
								Заказчик    <asp:Label ID="Label10" runat="server" Text='<%# (Eval("RecipientFirstName").ToString()) %>' /> &nbsp;
											<asp:Label ID="Label17" runat="server" Text='<%# (Eval("RecipientLastName").ToString()) %>' /> &nbsp;
											<asp:Label ID="Label18" runat="server" Text='<%# (Eval("RecipientThirdName").ToString()) %>' />
							</div>
							...
							<grb:GoodsList id="GoodsList" runat="server" Number='<%# Eval("BoxesNewNumber").ToString() %>' Cost='<%# Eval("AllCost").ToString() %>' TicketID='<%# Eval("ID").ToString() %>'/>
							...
							<table runat="server" id="Table1" class="printZP" style="width: 100%">
								...
								<tr>
									...
									<td  style="text-align: right">
										<%# 
											MoneyMethods.MoneySeparator(
                                                Convert.ToDecimal(MoneyMethods.GruzobozCostLoweringPercentage(Eval("GruzobozCost").ToString())) / Convert.ToDecimal(1  + Convert.ToInt32(StavkaNDS)/ 100)
                                            )
										%>
									</td>
									...
									<td style="text-align: right">
										 <%# 
                                             MoneyMethods.MoneySeparator(
													Convert.ToDecimal(MoneyMethods.GruzobozCostLoweringPercentage(Eval("GruzobozCost").ToString())) - Convert.ToDecimal(MoneyMethods.GruzobozCostLoweringPercentage(Eval("GruzobozCost").ToString())) / Convert.ToDecimal(1  + Convert.ToInt32(StavkaNDS)/ 100)
											)
                                         %>
									</td>
									<td style="text-align: right">
										<%# 
                                            MoneyMethods.MoneySeparator(
												MoneyMethods.GruzobozCostLoweringPercentage(Eval("GruzobozCost").ToString())
											)
                                        %>
									</td>
								</tr>
							...
							<div style="margin-top: 0px;">
								<div style="display: inline-block; width: 13cm; vertical-align: top;padding-top: 5px">
									Всего стоимость услуг с НДС <u><%# MoneyHelper.ToRussianString(Convert.ToDouble(MoneyMethods.GruzobozCostLoweringPercentage(Eval("GruzobozCost").ToString()))) %></u>
								</div>
