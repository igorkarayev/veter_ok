...
{
    public partial class PrintActORT : UserBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(idListString))
            {
                ...
                {
                    LabelID.Text = userInSession.ID.ToString();
                }
                ...
                //ds.Tables[0].Columns.Add("SenderCity", typeof(String));
                //ds.Tables[0].Columns.Add("Date", typeof(String));
                //ds.Tables[0].Columns.Add("SendDate", typeof(String));
                ds.Tables[0].Columns.Add("Goods", typeof(String));
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    ...
                    //row["Sender"] = OtherMethods.GetProfileData(row["UserProfileID"].ToString()) + System.Environment.NewLine + OtherMethods.GetProfileContactPhone(row["UserProfileID"].ToString());

                    //row["SenderCity"] = CityHelper.CityIDToCityName(row["CityID"].ToString());

                    //row["Date"] = OtherMethods.DateConvert(row["CreateDate"].ToString());

                    //row["SendDate"] = OtherMethods.DateConvert(row["DeliveryDate"].ToString());

                    row["BoxesNumber"] = row["BoxesNumber"] = false ? 1 : row["BoxesNumber"];
                    ...
                    {
                        string s1 = rowGoods["Description"].ToString();
                        string s2 = rowGoods["Model"].ToString();
                        string s3 = rowGoods["Number"].ToString();
                        row["Goods"] = string.Concat(new string[] { s1, ", ", s2, ", ", s3, "шт.;", "\n" });
                    }
                ...
                LabelSum.Text = costSum.ToString("0.00");
