...
{
    public partial class PrintPut1 : ManagerBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(idListString))
            {
                ...
                var fullSqlString =
                    "SELECT DISTINCT `Name` FROM `city` C JOIN `tickets` T ON C.`ID` = T.`CityID` WHERE (" +
                    sqlString.Remove(sqlString.Length - 3) + ")";
                var fullSqlString2 = "SELECT DISTINCT `CityID`, `DriverID` FROM `tickets` T WHERE (" +
                                     sqlString.Remove(sqlString.Length - 3) + ")";
                ...
                {
                    CityStringToHTML +=
                        "<tr><td></td><td></td><td>Минск</td><td style=\"text-align: left; padding-left: 5px;\">" + city +
                        "</td><td></td><td>Доставки</td><td>-</td><td>-</td></tr>";
                    RowSpanNumb++;
                }
