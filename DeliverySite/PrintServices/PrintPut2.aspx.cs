...
{
    public partial class PrintPut2 : ManagerBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(idListString))
            {
                ...
                var fullSqlString = "SELECT DISTINCT `Name` FROM `city` C JOIN `tickets` T ON C.`ID` = T.`CityID` WHERE (" + sqlString.Remove(sqlString.Length - 3) + ") AND T.`PrintNakl` = 1";
                var fullSqlString2 = "SELECT DISTINCT `CityID`, `DriverID` FROM `tickets` T WHERE (" + sqlString.Remove(sqlString.Length - 3) + ") AND T.`PrintNakl` = 1";
