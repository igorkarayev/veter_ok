...
{
    public partial class UserTicketByDeliveryOnMinsk : ManagerBasePage
    {
        ...
        {
            Page.Title = PagesTitles.ManagerUserTicketByDeliveryOnMinsk + BackendHelper.TagToValue("page_title_part");
            OtherMethods.ActiveRightMenuStyleChanche("hlTickets", this.Page);
            OtherMethods.ActiveRightMenuStyleChanche("hlUserTicketByDeliveryOnMinsk", this.Page);
            ...
            if (currentRole.PageUserTicketByDeliveryOnMinsk != 1)
            {
            ...
            btnSendToRouteRu.Visible =
                btnGetFromRouteRu.Visible =
                    btnDownloadXmlForRouteRu.Visible = sendToRouteAccessArray.Split(new[] { ',' }).Any(p => p.Trim().Contains(userInSession.ID.ToString())); ;
            ...
            if (ds.Tables[0].Rows.Count < Convert.ToInt32(BackendHelper.TagToValue("city_list_visible_count_limit")))
            {
                ...
                if (IsVisibleCityOrder)
                {
                    ...
                    if (driver.CityOrder != null)
                    {
                        ...
                        {
                            var firstOrDefault = citiInOrder.FirstOrDefault(u => u.cityid == Convert.ToInt32(row["CityID"]));
        ...
        public String GetSearchString(string sortExpression)
        {
            ...
            seletedCityString = seletedCityString + "(T.CityID in (select id from city where DistrictID in (10, 14, 2, 8, 16, 11, 3, 5, 19, 20, 21 ,1)))";
            ...
            var overSearchString = searchString.Length < 4
                    ? String.Format("SELECT {1} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID WHERE 1=1 {0}", sortExpression, selectFields)
                    : String.Format("SELECT {2} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID  WHERE {0} {1}", searchString.Remove(searchString.Length - 4), sortExpression, selectFields);

            DriverSearchString = searchString.Length < 4
                    ? "SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID"
                    : String.Format("SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID  WHERE {0}", searchString.Remove(searchString.Length - 4));
