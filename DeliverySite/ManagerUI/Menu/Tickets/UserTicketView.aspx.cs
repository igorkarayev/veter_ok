...
{
    public partial class UserTicketView : ManagerBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            OtherMethods.ActiveRightMenuStyleChanche("hlTicketsAll", this.Page);
            AppKey = Globals.Settings.AppServiceSecureKey;

            lblStatusSmsSuccess.Value = BackendHelper.TagToValue("sms-user-cabinet");
            ...
            if (currentRole.PageUserTicketView != 1)
            {
            ...
            if (!IsPostBack)
            {
                ...
                ddlAction.Items.Add("Печать акта выполненных услуг");
                ...
                //if (currentRole.ActionPrintAORT == 1)
                //{
                    ddlAction.Items.Add("Печать акта возврата");
                //}

                ddlAction.Items.Add("Отправить в BTS");
                #endregion
                ...
                DropDownListPaging.DataSource = DAL.DataBaseObjects.Tickets.Paging;
                DropDownListPaging.DataTextField = "Value";
                DropDownListPaging.DataValueField = "Key";
                DropDownListPaging.DataBind();
                ...
                sddlProfileType.Items.Insert(0, new ListItem("Все", string.Empty));
                ...
                if (!string.IsNullOrEmpty(Page.Request.Params["stateSave"]))
                {
                    ...
                    if (!string.IsNullOrEmpty(Page.Request.Params["dropDownListPaging"]))
                    {
                        DropDownListPaging.SelectedValue = Page.Request.Params["dropDownListPaging"];
                        lvDataPager.PageSize = Convert.ToInt32(DropDownListPaging.SelectedValue);
                    }
                    ...
                    if (!string.IsNullOrEmpty(Page.Request.Params["driverID"]) && sddlProfileType.Items.FindByValue(Page.Request.Params["driverID"]) != null)
                    {
                        sddlProfileType.SelectedValue = Page.Request.Params["driverID"];
                    }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            if(!IsPostBack)
            {
                ...
                if(user.ID.ToString() == "72" || user.ID.ToString() == "247" || user.ID.ToString() == "225")
                {
                    stbDeliveryDate1.Text = DateTime.Now.AddDays(-150).ToString("dd-MM-yyyy");
            ...
            var ds1 =
                dm.QueryWithReturnDataSet("SELECT ID, FirstName, LastName, ThirdName FROM drivers WHERE ID IN (" +
                                            DriverSearchString + ") ORDER BY FirstName");
            ds1.Tables[0].Columns.Add("FIOAndID", typeof(string),
                "'(' + ID + ') ' + FirstName + ' ' + SUBSTRING(LastName,1,1) + '.' +  SUBSTRING(ThirdName,1,1) + '.'");
            ...
            ds2.Tables[0].Columns.Add("FIOAndID", typeof(string),
                "'(' + ID + ') ' + FirstName + ' ' + SUBSTRING(LastName,1,1) + '.' +  SUBSTRING(ThirdName,1,1) + '.'");
        ...
        protected void btnAction_Click(object sender, EventArgs e)
        {
            ...
            if (ddlAction.SelectedValue == "Печать карты (кур.)"
                ...
                || ddlAction.SelectedValue == "Печать акта возврата"
                || ddlAction.SelectedValue == "Печать акта выполненных услуг")
            {
                ...
                if (ddlAction.SelectedValue == "Печать акта выполненных услуг")
                {
                    Response.Redirect(String.Format("~/PrintServices/PrintActRequisites.aspx?id={0}", idList));
                }
                ...
                {
                    Response.Redirect(String.Format("~/PrintServices/PrintCheck.aspx?id={0}&page=userticketsview&{1}", idList, OtherMethods.LinkBuilder(stbID.Text,
                ...
                if (ddlAction.SelectedValue == "Печать акта возврата")
                {
                    Response.Redirect(String.Format("~/PrintServices/PrintActReturn.aspx?id={0}", idList));
                }
            }

            if (ddlAction.SelectedValue == "Отправить в BTS")
            {
                ...
                var navSOAP = new NavSOAP();
                var obj = new SOAPPoint[idList.Count];
                int iterator = 0;
                foreach (var i in idList)
                {
                    var ticket = new DAL.DataBaseObjects.Tickets { ID = Convert.ToInt32(i) };
                    ticket.GetById();

                    var city = new DAL.DataBaseObjects.City { ID = Convert.ToInt32(ticket.CityID) };
                    city.GetById();

                    var goods = new Goods { TicketFullSecureID = ticket.FullSecureID };
                    var ds = goods.GetAllItems("ID", "ASC", "TicketFullSecureID").Tables[0].Rows;

                    obj[iterator] = new SOAPPoint();
                    obj[iterator].id = Guid.NewGuid().ToString();
                    obj[iterator].address = city.Name + ", " + ticket.RecipientStreetPrefix + " " + ticket.RecipientStreet + ", д. " + ticket.RecipientStreetNumber + (ticket.RecipientKorpus != "" ? " к. " + ticket.RecipientKorpus : "");
                    obj[iterator].label = ticket.Note;
                    obj[iterator].orderNumber = ticket.FullSecureID;
                    obj[iterator].phoneNumbers = ticket.RecipientPhone;

                    var res = navSOAP.CallWebService("addPoint", 3, obj[iterator]);
                    iterator++;
                }
                // Response.Redirect(String.Format("~/PrintServices/PrintTickets.aspx?id={0}", idList));
            }
        ...
        protected void btnConfirmUpload_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewDataItem items in lvAllTickets.Items)
            {
                ...
                if (chkBoxRows.Checked)
                {
                    ...
                    var currentStatusId = (HiddenField)items.FindControl("hfStatusID");
                    var ticket = new DAL.DataBaseObjects.Tickets { ID = Convert.ToInt32(ticketId.Value) };
                    if (currentStatusId.Value == "19")
                    {
                        var errorText = TicketsFilter.StatusChangeFilter(ref ticket, currentDriverId.Value, currentStatusId.Value, currentStatusDescription.Value, currentAdmissionDate.Value, tbStatusDescription.Text, "3", tbDeliveryDate.Text, currentRole);
                        if (errorText == null) //если ошибок после фильтрации нет - сохраняем заявку
                            ticket.Update(userInSession.ID, OtherMethods.GetIPAddress(), "UserTicketView");
                        else //выводим все ошибки, если они есть
                            lblNotif.Text += String.Format("{0}<br/>", errorText);
                    }
                }
            }
            Session["flash:now"] = "Загрузка подтверждена. Все заявки в статусе \"К загрузке\" переведены в статус \"В пути\".";
        }
        ...
        {
            if (!string.IsNullOrEmpty(DropDownListPaging.SelectedValue))
            {
                lvDataPager.PageSize = Convert.ToInt32(DropDownListPaging.SelectedItem.Text);
            }
            else
            {
                lvDataPager.PageSize = 100;
            }
            var dm = new DataManager();
            ...
            #region Скрытие столбцов в зависимоти от роли
            if (lvAllTickets.FindControl("CellTableND") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableND")).CssClass = UserRoles.TableND == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableProfile") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableProfile")).CssClass = UserRoles.TableProfile == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableGoods") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableGoods")).CssClass = UserRoles.TableGoods == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableKK") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableKK")).CssClass = UserRoles.TableKK == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableSendFrom") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableSendFrom")).CssClass = UserRoles.TableSendFrom == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableDirection") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableDirection")).CssClass = UserRoles.TableDirection == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableRecieverCity") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableRecieverCity")).CssClass = UserRoles.TableRecieverCity == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableRecieverAddr") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableRecieverAddr")).CssClass = UserRoles.TableRecieverAddr == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableReciever") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableReciever")).CssClass = UserRoles.TableReciever == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableNotes") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableNotes")).CssClass = UserRoles.TableNotes == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableComments") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableComments")).CssClass = UserRoles.TableComments == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableOtherOptions") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableOtherOptions")).CssClass = UserRoles.TableOtherOptions == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableRecieveDate") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableRecieveDate")).CssClass = UserRoles.TableRecieveDate == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableSendDate") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableSendDate")).CssClass = UserRoles.TableSendDate == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableStatus") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableStatus")).CssClass = UserRoles.TableStatus == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableDID") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableDID")).CssClass = UserRoles.TableDID == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableAgreedCost") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableAgreedCost")).CssClass = UserRoles.TableAgreedCost == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableCost") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableCost")).CssClass = UserRoles.TableCost == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTablePN") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTablePN")).CssClass = UserRoles.TablePN == 1 ? "displayColumn" : "notDisplayColumn";

            if (lvAllTickets.FindControl("CellTableWeight") != null)
                ((TableHeaderCell)lvAllTickets.FindControl("CellTableWeight")).CssClass = UserRoles.TableWeight == 1 ? "displayColumn" : "notDisplayColumn";
            #endregion
            ...
            if (ds.Tables[0].Rows.Count < Convert.ToInt32(BackendHelper.TagToValue("city_list_visible_count_limit")))
            {
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    ...
                    if (!contains && row["CityID"].ToString() != "-1" && row["CityID"].ToString() != "0")
                    {
                        ...
                        if (row["CityTrackID"] == DBNull.Value)
                            newRow["TrackName"] = String.Empty;
                        if (row["CityTrackID"] != DBNull.Value)
        ...
        public String GetSearchString(string sortExpression)
        {
            ...
            var searchNotFromKalvariiskaia = String.Empty;
            ...
            //формируем строку поика для типов пользователей
            foreach (var selectedIndex in sddlProfileType.GetSelectedIndices())
            {
                if (!string.IsNullOrEmpty(sddlProfileType.Items[selectedIndex].Value))
                {
                    if (sddlProfileType.Items[selectedIndex].Value == "0")
                    {
                        searchTypeIdString = searchTypeIdString + "U.TypeID = '" +
                                             sddlProfileType.Items[selectedIndex].Value + "' OR U.TypeID = -1 OR ";
                    }
                    else
                    {
                        searchTypeIdString = searchTypeIdString + "U.TypeID = '" + sddlProfileType.Items[selectedIndex].Value + "' OR ";
                    }
                }
            }
            if (searchTypeIdString.Length > 3)
            {
                searchTypeIdString = "(" + searchTypeIdString.Remove(searchTypeIdString.Length - 3) + ")";
            }
            ...
            {
                //searchTicketIdString = "T.SecureID CONTAINS '" + stbID.Text + "'";
                searchTicketIdString = "T.SecureID LIKE '%" + stbID.Text + "%'"; 
            }
            ...
            //формируем cтроку для поиска по Billed
            if (cbNotFromKalvariiskaia.Checked)
            {
                searchNotFromKalvariiskaia = "(T.`SenderStreetName` != '" + BackendHelper.TagToValue("loading_point_street") + "' AND T.`SenderStreetNumber` != '" + BackendHelper.TagToValue("loading_point_street_number") + "')";
            }
            ...
            const string selectFields = "T.WithoutMoney, " +
                                        ...
                                        "T.OvDateTo";
            var overSearchString = String.Empty;
            if (!cbTicketWithOldGoods.Checked)
                overSearchString = searchString.Length < 4
                    ? String.Format("SELECT {1} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID WHERE {0}", sortExpression, selectFields)
                    : String.Format("SELECT {2} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID WHERE {0} {1}", searchString.Remove(searchString.Length - 4), sortExpression, selectFields);
            else
                overSearchString = searchString.Length < 4
                    ? String.Format("SELECT {1} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID JOIN `goods` as G on G.TicketFullSecureID = T.FullSecureID WHERE G.Description = (SELECT Name FROM titles WHERE Name = G.Description AND CanBeWithoutAkciza = 1) GROUP BY G.TicketFullSecureID {0}", sortExpression, selectFields)
                    : String.Format("SELECT {2} FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID JOIN `goods` as G on G.TicketFullSecureID = T.FullSecureID WHERE G.Description = (SELECT Name FROM titles WHERE Name = G.Description AND CanBeWithoutAkciza = 1) AND {0} GROUP BY G.TicketFullSecureID {1}", searchString.Remove(searchString.Length - 4), sortExpression, selectFields);

            if (!cbTicketWithOldGoods.Checked)
                DriverSearchString = searchString.Length < 4
                    ? String.Format("SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID")
                    : String.Format("SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID WHERE {0}", searchString.Remove(searchString.Length - 4));
            else
                DriverSearchString = searchString.Length < 4
                    ? String.Format("SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID JOIN `goods` as G on G.TicketFullSecureID = T.FullSecureID WHERE G.Description = (SELECT Name FROM titles WHERE Name = G.Description AND CanBeWithoutAkciza = 1) GROUP BY G.TicketFullSecureID")
                    : String.Format("SELECT DISTINCT T.`DriverID` FROM `tickets` as T JOIN `city` as C on T.CityID = C.ID JOIN `usersprofiles` as U on T.UserProfileID = U.ID JOIN `goods` as G on G.TicketFullSecureID = T.FullSecureID WHERE G.Description = (SELECT Name FROM titles WHERE Name = G.Description AND CanBeWithoutAkciza = 1) AND {0} GROUP BY G.TicketFullSecureID", searchString.Remove(searchString.Length - 4));
